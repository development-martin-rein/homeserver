...
run_btop() {
  require_cmd btop && btop
}

run_htop() {
  require_cmd htop && htop
}

run_stui() {
  require_cmd s-tui && s-tui
}

run_atop() {
  require_cmd atop && sudo atop
}

run_csysdig() {
  require_cmd csysdig && sudo csysdig
}

run_nvtop() {
  require_cmd nvtop && nvtop
}

run_perf_top() {
  require_cmd perf && sudo perf top
}
...
run_iftop_default() {
  ...
  log "Optionen: keine Domain Name System Auflösung (-n), Ports anzeigen (-P)"
...
atop_logs() {
  # Debian/Ubuntu: /var/log/atop/atop_YYYYMMDD
...
enable_atop_logging() {
  if systemctl list-unit-files | grep -qE '^atop\.service'; then
    ...
    echo
    log "Status:"
    systemctl --no-pager status atop || true
  else
